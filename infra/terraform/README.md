...
The scheduler supplies only the execution timestamp and global lookback. The
state machine attaches the exact S3 version and SHA-256 of the independently
deployed target manifest before invoking discovery. Discovery returns
provider-neutral filing references plus target-set provenance. A bounded inline
`Map` invokes acquisition and then normalization for each ready filing, passing
the workflow execution ARN as both claim owners. Filing bodies never enter
workflow state.
...
- a source-controlled company/regulator target set deployed to a private,
  versioned S3 configuration bucket and read by exact object version;
...
# Replace the example AWS account ID and SEC contact address. Review
# ../../config/discovery-targets/dev.json before continuing.
...
aws stepfunctions start-execution \
  ...
  --input '{"filed_from":"2025-01-01","filed_to":"2025-12-31"}'
```

Inspect the execution in Step Functions and confirm its output contains
`target_set`, `issuers_scanned`, `filings_found`, and one `filing_results` entry
per filing. `target_set` must identify the expected logical revision, S3 object
version, and SHA-256.
...
each invocation. It does not carry company or filing-type configuration.
Step Functions pins the deployed target version, and the Lambda derives and
logs the inclusive discovery window shared by every target.
...
All three S3 buckets block every form of public access, disable ACL-based
ownership, deny non-TLS requests, and apply SSE-S3 encryption. Versioning
protects against accidental overwrites. Target-config versions are retained for
execution provenance. Current corpus objects do not expire; their noncurrent
versions expire after 30 days, incomplete multipart uploads after seven days,
and orphaned delete markers are removed. Generated names contain the AWS account
ID and a stable environment/Region hash.
...
Change companies, regulator registrations, or filing types in
`config/discovery-targets/<environment>.json`; increment its `revision`, then
plan and apply. Change the global lookback, frequency, or log retention through
Terraform variables. To stop recurring work without deleting resources, set
`schedule_enabled = false` and apply. To remove the whole slice:
...
the registry table. Nonempty target-config, raw, and normalized buckets also
block destruction by default. Set the corresponding `*_bucket_force_destroy`
variable only when every object version in an explicitly disposable environment
may be deleted.
